...
- **工具层**(tool layer)：各种各样的编辑器（这也是我们对游戏引擎的第一印象）

    <div style="text-align: center">
        <img src="images/lec2/1.png" width=70%>
    </div>
...
>注：层级的划分并没有一个统一的标准，这里采取的是一种比较简单且易于理解的划分。
...
- 离线资源导入(offline resource importing)（资源(resource) -> **资产**(asset)）
    ...
    - 通过**导入预处理**(importing preprocess)，可以加速对资产的访问
    - 构建一个**复合资产文件**(composite asset file)来引用(reference)（或者说关联）所有的资源
...
- **tick**
    ...
    - tick 的两大组成部分：
        ...
        - 一般而言先做完逻辑再做渲染，如果反过来的话画面看上去会有延迟感（比如输入要到下一个 tick 才能通过屏幕反映出来）
