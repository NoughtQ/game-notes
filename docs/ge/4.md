...
<div style="text-align: center">
    <img src="images/lec4/1.png" width=70%>
</div>
...
- [**投影**](../cg/3.md#projection-transformations)(projection)和[**光栅化**](../cg/4.md)(rasterization)
...
- [**纹理采样**](../cg/5.md#texture-mapping)(texture sampling)
...
在游戏引擎中，最基础的表达形式是**网格图元**(mesh primitive)，比如下面对顶点、三角形的定义：
...
<div style="text-align: center">
    <img src="images/lec4/29.png" width=80%>
</div>
...
- 包围球
- **AABB**（轴对齐包围盒(axis-aligned boudning box)）：
    - 「轴」是指世界（全局）坐标系的 x, y, z 轴
    - 只要存（对角线上）两个顶点就能构建出来，且计算效率很高，仅次于包围球
...
另一种可见性裁剪算法是 **PVS**（潜在可见集(potentially visible set)）。简单来说就是将空间划分为多个小格子（房间），格子之间通过大门(portal)相连。
