...
- 直接在 2D 表面上追踪光线，并且从地平角近似表示 AO
    - 用到之前介绍的射线步进算法，但每次移动都要稍微改变一点方向，否则由于采样率低，会看到一些瑕疵
...
前面介绍的两种方法都是上古时期的做法。现代的方法是采用**基于体素的体积雾**(voxel-based volumetric fog)。
...
???+ example "例子"
    ...
    === "例2"
        ...
        >由于 gif 文件太大，我不得不将其压缩，所以这个 gif 图画质很差，完全没法体现反走样的效果，请见谅🙏
...
**渲染管线**(rendering pipeline)是指所有渲染操作执行和资源分配的管理顺序。
...
我们称这样的管线为**前向渲染**(forward rendering)，可以用以下简短的伪代码表达：
...
但上述这套管线难以应对现代游戏中同一场景下存在大量不同光源的情况。
...
在移动端中，更常见的渲染管线是**基于块的管线**(tile-based rendering)，因为移动端需要考虑存储芯片（DRAM）的发热问题。
...
该算法带来的额外好处是同时将光源进行切割，将其划分到一个个块中，这样方便区分某个块中存在哪些光照（被记录在**光照列表**(light list)中）。这种技术称为**光照剔除**(light culling)。
...
---
基于块的渲染可以和前向渲染结合，得到新的管线（即**基于块的前向渲染**）。它的特点是：
...
- 将管线中的所有东西都变成一个个模块(modules)，在图中可拖拽
